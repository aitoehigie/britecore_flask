...
def create():
    ...
    if request.method == "GET":
        ...
        if title:
            new_feature = FeatureRequest(title=title,
...
def reOrderPriority(priority,client):
    ...
    features_to_update_count = FeatureRequest.query.filter_by(client=client, client_priority=tempPriority).count()

    if features_to_update_count > 1:
        feature_to_update = FeatureRequest.query.filter_by(client=client, client_priority=tempPriority).order_by('id').first()
