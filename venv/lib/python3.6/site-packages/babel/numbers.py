...
def format_decimal(number, format=None, locale=LC_NUMERIC, decimal_quantization=True):
...
def parse_decimal(string, locale=LC_NUMERIC):
    ...
    try:
        return decimal.Decimal(
            string.replace(get_group_symbol(locale), "").replace(
                get_decimal_symbol(locale), "."
            )
        )
