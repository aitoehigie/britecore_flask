...
class BaseView(with_metaclass(AdminViewMeta, BaseViewClass)):
    ...
    def __init__(
        ...
        if self._default_view is None:
            raise Exception(
                u"Attempted to instantiate admin view %s without default view"
    ...
    def _init_extension(self):
        ...
        for p in admins:
            if p.endpoint == self.endpoint:
                raise Exception(
                    u"Cannot have two Admin() instances with same" u" endpoint name."
                )

            if p.url == self.url and p.subdomain == self.subdomain:
                raise Exception(
                    u"Cannot assign two Admin() instances with same"
