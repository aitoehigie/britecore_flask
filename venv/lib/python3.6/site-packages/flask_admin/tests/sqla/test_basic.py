...
def test_model():
    ...
    rv = client.post(
        "/admin/model1/new/",
        data=dict(test1="test1large", test2="test2", time_field=time(0, 0, 0)),
    )
...
def test_column_editable_list():
    ...
    rv = client.post(
        ...
        data={"list_form_pk": "1", "test1": "change-success-1"},
    )
    data = rv.data.decode("utf-8")
    ...
    rv = client.post(
        "/admin/model1/ajax/update/",
        data={"list_form_pk": "1", "enum_field": "problematic-input"},
    )
    ...
    rv = client.post(
        "/admin/model1/ajax/update/",
        data={"list_form_pk": "1000", "test1": "problematic-input"},
    )
    data = rv.data.decode("utf-8")
    eq_(rv.status_code, 500)
    ...
    rv = client.post(
        "/admin/model1/ajax/update/",
        data={"list_form_pk": "1", "test2": "problematic-input"},
    )
    data = rv.data.decode("utf-8")
    ...
    rv = client.post(
        "/admin/model2/ajax/update/", data={"list_form_pk": "1", "model1": "3"}
    )
    data = rv.data.decode("utf-8")
    ok_("Record was successfully saved." == data)
...
def test_editable_list_special_pks():
    ...
    rv = client.post(
        "/admin/model1/ajax/update/",
        data={"list_form_pk": "1-1", "val1": "change-success-1"},
    )
    data = rv.data.decode("utf-8")
    ok_("Record was successfully saved." == data)
...
def test_column_filters():
    ...
    view = CustomModelView(Model2, db.session, column_filters=["model1"])
    ...
    eq_(
        [
            (f["index"], f["operation"])
            for f in view._filter_groups[u"Model1 / Bool Field"]
        ],
        [(28, u"equals"), (29, u"not equal")],
    )

    eq_(
        [
            (f["index"], f["operation"])
            for f in view._filter_groups[u"Model1 / Enum Field"]
        ],
        [
            (30, u"equals"),
            (31, u"not equal"),
    ...
    view = CustomModelView(Model2, db.session, column_filters=["model1.bool_field"])

    eq_(
        [
            (f["index"], f["operation"])
            for f in view._filter_groups[u"model1 / Model1 / Bool Field"]
        ],
        [(0, "equals"), (1, "not equal")],
    )
    ...
    # Test integer filter
    view = CustomModelView(Model2, db.session, column_filters=["int_field"])
    admin.add_view(view)
    ...
    view = CustomModelView(
        ...
        column_list=["string_field", "model1.id", "model1.bool_field"],
    )
    admin.add_view(view)
...
def test_form_columns():
    ...
    view1 = CustomModelView(
        Model, db.session, endpoint="view1", form_columns=("int_field", "text_field")
    )
    view2 = CustomModelView(
        Model, db.session, endpoint="view2", form_excluded_columns=("excluded_column",)
    )
...
def test_form_override():
    ...
    view2 = CustomModelView(
        Model, db.session, endpoint="view2", form_overrides=dict(test=fields.FileField)
    )
...
def test_modelview_named_filter_localization():
    ...
    view = CustomModelView(
        Model1, db.session, named_filter_urls=True, column_filters=["test1"]
    )
