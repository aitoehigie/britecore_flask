...
def test_column_editable_list():
    ...
    rv = client.post(
        ...
        data={"list_form_pk": str(obj1.id), "test1": "change-success-1"},
    )
    data = rv.data.decode("utf-8")
    ...
    rv = client.post(
        "/admin/model1/ajax/update/",
        data={"list_form_pk": str(obj2.id), "datetime_field": "problematic-input"},
    )
    ...
    rv = client.post(
        ...
        data={"list_form_pk": str(obj3.id), "model1": str(obj1.id)},
    )
    data = rv.data.decode("utf-8")
    ok_("Record was successfully saved." == data)
...
def test_column_filters():
    ...
    # Test boolean filter
    view = CustomModelView(Model2, column_filters=["bool_field"], endpoint="_bools")
    admin.add_view(view)
    ...
    # Test float filter
    view = CustomModelView(Model2, column_filters=["float_field"], endpoint="_float")
    admin.add_view(view)
    ...
    view = CustomModelView(
        Model1, column_filters=["datetime_field"], endpoint="_datetime"
    )
    admin.add_view(view)
    ...
    # datetime - between
    rv = client.get(
        "/admin/_datetime/?flt0_4=2014-04-02+00%3A00%3A00+to+2014-11-20+23%3A59%3A59"
    )
    eq_(rv.status_code, 200)
    data = rv.data.decode("utf-8")
    ok_("datetime_obj1" in data)
    ...
    rv = client.get(
        "/admin/_datetime/?flt0_5=2014-04-02+00%3A00%3A00+to+2014-11-20+23%3A59%3A59"
    )
    eq_(rv.status_code, 200)
    data = rv.data.decode("utf-8")
    ok_("datetime_obj1" not in data)
...
def test_subdocument_config():
    ...
    view1 = CustomModelView(
        Model1, form_subdocuments={"subdoc": {"form_columns": ("name",)}}
    )
    ...
    view2 = CustomModelView(
        Model1, form_subdocuments={"subdoc": {"form_excluded_columns": ("value",)}}
    )
...
def test_subdocument_class_config():
    ...
    # Check only
    view1 = CustomModelView(Model1, form_subdocuments={"subdoc": EmbeddedConfig()})
...
def test_nested_subdocument_config():
    ...
    view1 = CustomModelView(
        Model1,
        form_subdocuments={
            "nested": {"form_subdocuments": {"comment": {"form_columns": ("name",)}}}
        },
    )
...
def test_nested_ajax_refs():
    ...
    view1 = CustomModelView(
        Model1,
        form_subdocuments={
            "nested": {"form_ajax_refs": {"comment": {"fields": ["name"]}}}
        },
    )
