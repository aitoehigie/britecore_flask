...
class BoolNaCol(OptCol):
    ...
    def __init__(
        self, name, yes_display=None, no_display=None, na_display=None, **kwargs
    ):
        if yes_display is None:
            yes_display = self.yes_display
        if no_display is None:
...
class LinkCol(Col):
    ...
    def __init__(
        ...
        attr_list=None,
        url_kwargs=None,
        url_kwargs_extra=None,
        anchor_attrs=None,
        text_fallback=None,
        **kwargs
    ):
        super(LinkCol, self).__init__(name, attr=attr, attr_list=attr_list, **kwargs)
    ...
    def url_kwargs(self, item):
        ...
        item_kwargs = {
            k: _recursive_getattr(item, v) for k, v in self._url_kwargs.items()
        }
...
class ButtonCol(LinkCol):
    ...
    def td_contents(self, item, attr_list):
        ...
        button = element(
            "button", attrs=button_attrs, content=self.text(item, attr_list)
        )
        form_attrs = dict(self.form_attrs)
        form_attrs.update(dict(method="post", action=self.url(item)))
        form_hidden_fields_elements = [
            element("input", attrs=dict(type="hidden", name=name, value=value))
            for name, value in sorted(self.form_hidden_fields.items())
        ]
        return element(
            "form",
            attrs=form_attrs,
            content=["".join(form_hidden_fields_elements), button],
