...
class Table(with_metaclass(TableMeta)):
    ...
    def __html__(self):
        ...
        if tbody or self.allow_empty:
            content = "\n{thead}\n{tbody}\n".format(thead=self.thead(), tbody=tbody)
            return element(
                "table",
    ...
    def thead(self):
        ths = "".join(
            self.th(col_key, col) for col_key, col in self._cols.items() if col.show
        )
        content = element("tr", content=ths, escape_content=False)
        return element(
            "thead", attrs=self.get_thead_attrs(), content=content, escape_content=False
        )
