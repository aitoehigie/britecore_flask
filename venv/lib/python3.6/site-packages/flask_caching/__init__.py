...
class Cache(object):
    ...
    def _memoize_version(
        self, f, args=None, reset=False, delete=False, timeout=None, forced_update=False
    ):
    ...
    def _memoize_make_cache_key(
        self, make_name=None, timeout=None, forced_update=False
    ):
    ...
    def _memoize_kwargs_to_args(self, f, *args, **kwargs):
        ...
        return (
            tuple(new_args),
            OrderedDict(
                sorted((k, v) for k, v in iteritems(kwargs) if k in kw_keys_remaining)
            ),
        )
